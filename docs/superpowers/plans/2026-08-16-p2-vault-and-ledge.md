...
**不给坐标——给性质。** 我在这个靶场里手写坐标已经连续错了三次（区域互相重叠、垂直不连通、超出地面边界），所以本任务改为规定必须满足的性质，由执行者选坐标并用测试证明。

`VaultArea` 需要提供，按玩家跑动方向依次排列：

1. **三个高度递增的可翻越障碍**，最高的一个刚好在 `config.vault_max_height` 之内。它们要能被跑动中的玩家依次撞上，间距足够重新加速。
2. **一堵明确超过 `vault_max_height` 的墙**，用来验证"翻不过去"确实翻不过去。它不能挡住通往后半段的路——玩家要能绕过它。
3. **两个高度递增的可抓边缘平台**，分别落在 `ledge_min_height` 与 `ledge_max_height` 之间的两端附近。
4. **一个明确超过 `ledge_max_height` 的高台**，用来验证够不到。

硬性约束：

- **整个区域必须完全落在竞技场地面之内。** 现有地面是 60×60（`x`、`z` 各 ±30）。**如果放不下，就把地面加大**，不要让方体悬空到边界外——那会让玩家踩空掉下去，而 `Arena` 的坠落回收会把他送回出生点，练习区就废了。
- **不得与其他练习区重叠。** 已有 `JumpArea`（跳跃区，含 `x` 约 −10.5 到 +11 的平台）与 `SlideArea`（滑铲区，`x` 约 14 到 22）。跨区域重叠测试会捕获违规，但**先算清楚再生成**，不要靠测试发现。
- 沿用既有的 `_box()`、`_attach()`、`_material_for()` 辅助函数。

**这些数字要从 `config` 读，不要写死。** 障碍高度与 `vault_max_height` / `ledge_min_height` / `ledge_max_height` 的关系是这个区域存在的全部意义；写死数字之后，调参一改，区域就悄悄失去意义了。
