...
11. **测试里往场景加几何体时，必须先设 `position` 再 `add_child()`。** 本文档的测试代码里若出现"先 `add_child()`、等一帧、再设 `global_position`"的写法，**那是错的，请自行改正**——那一帧里物体停在世界原点，会与地面和玩家重叠并把玩家弹飞，产生与被测行为无关的假失败。用局部的 `position`（出树可用），不要用 `global_position`（出树会报错）。
12. **`CameraRig.update_effects()` 每帧硬性重置 `position.y = eye_height`。** 相机高度类的新效果必须实现成独立累加的偏移量；`rotation.z` 不受此影响，所以本阶段的 roll 倾斜可以直接写。
13. **入口类判定用按下沿，持续类判定用按住。** 测试里用 `press_crouch()` / `press_jump()` 而非直接写 `state.*_held`。
14. **状态脚本里不能对穿过 `player` 的表达式用 `:=` 推断类型。** `PlayerState.player` 是**无类型**的（为了避免 `class_name` 循环引用），所以 `var forward := -player.global_transform.basis.z` **无法编译**，而且报出的错误极具误导性（`Nonexistent function 'new'`），还会让整个测试套件一起失败。写成 `var forward: Vector3 = -player.global_transform.basis.z`，显式标注类型。P2 的攀爬状态正是栽在这一条上。
15. **入口判定必须以 `player.grounded` 为前提。** `GroundState` 里任何返回脚本化位移状态的分支，都在该帧调用 `move_and_slide()` 之前就返回了，因此不能依赖 `is_on_floor()`；未加守卫会让身体带着未经验证的位置连锁进入下一个脚本化位移。P2 已为翻越与滑铲加上守卫，本阶段新增的入口同样要加。
...
> **注意：下面这段 `@onready` 写法是过时的，不要照抄。** P2 实测发现 `Probes.setup()` 会在 `Probes._ready()` 之前运行（测试夹具的构造顺序所致），`@onready` 字段此时仍为 null，会直接崩。现有的 `probes.gd` 已全部改用实时 `get_node()` 查找——**按文件里现有的写法来**，不要引入 `@onready`。

```gdscript
# 过时示例，仅说明需要哪两个节点：
# _wall_left  -> $WallLeft
# _wall_right -> $WallRight
```
...
func enter(_previous: StringName) -> void:
	...
	# inferred — P2 replaced is_on_floor() as the authority precisely so that no
	# state can leave a stale value behind.
	#
	# CRITICAL: declare it EVERY TICK, not just here. P2's structural invariant
	# only checks "declared at least once since entry", so a state that declares
	# a stale value in enter() and never again would satisfy the guard while
	# lying for its entire duration — coyote time would refill for the whole
	# wall run and a buffered jump would fire off the wall.
...
**优先级必须刻意决定，不能靠行序继承。** P2 的收尾审查指出：`AirState` 里的抓边缘判定是无条件且排在最前的，所以蹬墙判定若排在它后面，**永远抢不过它**——贴着一面墙跳跃时，只要顺带够到了任何可抓边缘，就会变成攀爬而不是蹬墙。

本阶段要明确回答"同时满足两个条件时选哪个"，并把答案写成注释。建议的答案是**蹬墙优先**：抓边缘是"救回一次失误"的补救动作，而蹬墙是玩家主动选择的路线，玩家高速贴墙飞过时想要的是蹬墙。同时补一个测试，构造"墙面与可抓边缘同时在探测范围内"的几何，断言进入的是蹬墙而非攀爬。
...
**不给坐标——给性质。** 这个靶场的坐标我已经手写错了四次：区域互相重叠、课程垂直不连通、几何超出地面边界，以及本节原本的写法（`ZigLeft2` 在世界 `z=50`，而地面只到 ±30，`LongWall` 也有一半悬空）。所以本任务同样改为规定性质，由执行者选坐标并用测试证明。

`WallArea` 需要提供：

1. **一段助跑区**，让玩家能把速度堆到 `wall_min_speed` 之上再接触墙面。
2. **一面足够长的墙**，长到能跑满一次完整的蹬墙时长——长度至少要覆盖 `wall_max_speed × wall_max_duration` 的一半，否则测不出时长上限。
3. **一组 Z 字交错的墙**，相邻墙面**朝向相反**。这一点是这个区域存在的理由：法线相反意味着不会被同墙冷却挡住，玩家可以在两面墙之间接力上升——防刷机制要允许的正是这种玩法，禁止的是在同一面墙上反复蹬。
4. 墙面要**足够高**，让蹬墙期间玩家不会从顶上翻出去。

硬性约束（与前两个练习区相同）：

- **整个区域必须完全落在竞技场地面之内**（当前 60×60，`x`、`z` 各 ±30）。放不下就把地面加大，不要让任何几何悬空到边界外——玩家会踩空掉下去并被坠落回收送回出生点。
- **不得与 `JumpArea`、`SlideArea`、`VaultArea` 重叠。** 跨区域重叠测试会捕获违规，但先算清楚再生成。
- 长度类的判据要**从 `config` 推导**（例如长墙长度对照 `wall_max_speed × wall_max_duration`），不要写死。
