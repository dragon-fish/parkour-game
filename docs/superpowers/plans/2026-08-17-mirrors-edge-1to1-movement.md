...
Expected: 以**低于顶速**冲过 0.9 m 的箱子会被**补回** +0.8 m/s（⚠️ 在满速 7.2 时净收益为零——`clamp_speed_max` 恰等于 `ground_speed`，这是原版确证的咬合：这个奖励是「把损失的速度补回来」，不是「跑得比能跑的更快」），而且动作触发得更早、更顺；慢慢走过去则是「爬上去」，没有奖励。1.7 m 的高障碍**必须先按跳、在上升途中**才能翻，落地明显掉速且耗时约两倍。
