...
- `trigger_distance` 1.2 -> 2.4，`approach_timeout` 0.6 -> 1.0（owner 实测 1.2 太短，
  ...
  合法助跑判死）。`plant_reach` 0.55 -> 0.8：落脚点被"走离棱边"挪进立面 0.1-0.2 m，
  而胶囊半径又让脚停在立面外 0.4 m，两者之和得包得住。
- 助跑段除"进入 `plant_reach`"外，增加"身体被顶住就开蹬"（连续 3 帧水平位移
  < 0.005 m）。这是保险，不是当前画廊的必需项——见 task-5-report 的实测。
