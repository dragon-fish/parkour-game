...
节点名一律由生成器显式给出，**不得交给 Godot 自动命名**。同一父节点下重名时 Godot
会改成 `@Path3D@9002` 这类带 `@` 的名字：`@` 在节点名里不合法，引擎不会崩，但会刷
奇怪的警告，而且名字每次生成都可能变。现有教程关薄壳里已经有一个这样的铁丝网节点，
来源是两条原作记录重名。

- 原作对象名作为节点名前先清洗：只保留 Godot 节点名允许的字符。
- 同一父节点下重名时按出现顺序追加 `_2`、`_3`……，结果与生成次数无关。
