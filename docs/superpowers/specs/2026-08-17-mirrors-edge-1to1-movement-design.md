...
> **实施偏离**：上面「以及 roll 动画结束时」这一条**没有实现**，见 [§10.2](#102-fall_height-只在触地事件归零对应-35)。
...
> **实施偏离**：WallrunJump **没有**做成独立注册的 Move，参数与梯度公式照做，见 [§10.1](#101-wallrunjump-没有做成独立注册的-move对应-43--55)。
...
> **收尾状态（Task 16）**：第一条风险**仍未实测**——Task 11 Step 8 是手动试玩关卡，这一批没有产出任何连跳高度数字。详见 [§10.3](#103-8-的-zig-zag-连跳风险仍未实测)。另外，练习场 zig-zag 走廊本身的设计前提也存疑，见 [§10.4](#104-zig-zag-走廊的设计前提本身存疑关卡设计问题非缺陷)。
...
以下是 16 个 task 落地过程中，实现与本设计文档不一致的地方。**这些不是待办事项**——每一条都是已经落地并且经过权衡的决定，记在这里是为了将来读代码的人（人或 agent）不会把它们当成 bug「修」回去。

### 10.1 WallrunJump 没有做成独立注册的 Move（对应 §4.3 / §5.5）

设计里 WallrunJump 是一个独立的 Move。实现没有这么做（见 Task 12）：这个动作只活**一个 tick**——离墙瞬间写完速度就该交给 `FallingMove`。做成独立 Move 只有两种收场：要么在它自己身上多停一帧（凭空多出一帧的停顿），要么把 `FallingMove` 的全部逻辑（重力、着地判定、贴墙/翻越/抓边探测）复制一份进去。

实际做法：墙跳分支留在 `WallRunMove` 里，但**参数全部取自 `wallrun_jump` config**，梯度公式（§5.5 那四行）是一个可以单测的纯函数——`tests/test_wallrun_jump.gd` 直接钉住 noob/pro 两端与插值。丢掉的只是「注册表里多一个名字」，没丢任何行为或可测性。

### 10.2 `fall_height` 只在触地事件归零（对应 §3.5）

§3.5 写的归零条件有两个：触地事件，以及「roll 动画结束时」。实现只有前者。

原因：**本项目没有 Roll 这个独立动作状态**——skill roll 在这里是落地那一帧的一个布尔判定（`FallingMove` 的 `consume_roll()`），不是一段有开始和结束的动画状态，没有「动画结束」这个时刻可以挂。

实际影响很小：drop-roll 的效果本来就来自落地那次触地归零，第二个条件在原版里是「结算发生在动画结束而非落地瞬间」的证据，不是一条额外的归零规则。

### 10.3 §8 的 zig-zag 连跳风险：**仍未实测**

§8 记的风险（删掉 `wall_reattach_cooldown` 与 `_height_ceiling()` 之后，双墙 zig-zag 可能变成无限爬升）在这一批里**没有被量化**。Task 11 Step 8 是留给所有者的手动试玩关卡，不是自动化测试；Task 11 自己的报告写得很清楚：它的自动化测试对连跳高度「给不出任何数字」。

代码层面能确认的是：`WallRunMove` 的墙跳分支现在无条件写 `velocity.y = wallrun_jump.wall_jump_up`，不参考已经攒了多少高度；`redo_move_time` 又是**对整个 Move** 生效而非对同一面墙生效，所以两面相对的墙互不阻挡。**代码里没有任何东西限制连跳次数。**

→ 这条留给所有者：自测清单第 6 项就是它。在实测之前，§8 的风险状态是「未验证」，不是「已解决」。

### 10.4 zig-zag 走廊的设计前提本身存疑（关卡设计问题，非缺陷）

`tools/arena_builder.gd` 的 zig-zag 走廊要求玩家**同时够到两面墙**，于是走廊半宽被压到 0.45 m——探针 reach 0.5 减去胶囊半径 0.4，整个系统只剩 0.1 m 余量，每侧 0.05 m。

但原版过这种缺口是**在空中**完成的：跑一面墙 → 蹬开 → 飞过缺口 → 贴上对面的墙，飞行途中不需要够到任何东西。按那个模型，走廊宽度应该由**墙跳推开距离**决定，而不是由探针 reach 决定。本批次实测的墙跳推开距离是最差 1.2 m、最好 6.58 m——比 0.9 m 的走廊宽度大一个数量级。

这不是实现缺陷（当前几何与当前探针参数自洽），而是一个**关卡设计问题**：这段练习场到底想练哪个动作。记在这里给所有者定夺。

### 10.5 vault 表在甜区高度上有一个动量缺口（忠实转录，非移植错误）

在甜区障碍高度上存在一段接近速度带——大致 **2.0–4.0 m/s**——六个 vault 变体没有一个匹配：速度不够低走不了慢速变体，不够高又达不到快速变体的门槛。结果是玩家**直接撞停在障碍上，没有任何反馈**。

这是对原版 `VaultTypes` 表的**忠实转录**，不是移植时算错了。但它是一个真实存在的玩法空洞，所有者应当知道——补法有两种（放宽某个变体的速度下限，或在无匹配时给一个「撞停」的明确反馈），都需要偏离 1:1，所以留给所有者拍板。

### 10.6 Grab 的 `redo_move_time` 用的是项目值 0.45，不是原版的 0.15

§6 的删除表把 `ledge_regrab_cooldown` 映射到 `redo_move_time`，Task 16 落地了这个映射。但**值没有跟着换成原版的**：附录 A1 记的 `TdMove_Grab.RedoMoveTime = 0.15`，这里保留了本项目一直在玩的 0.45。

理由：本项目的 Grab 是**一个 Move 同时覆盖悬挂与翻上**，原版把这两段拆在 `TdMove_Grab` / `TdMove_GrabPullUp` / `TdMove_GrabTransfer`（后者自己的 `RedoMoveTime` 是 0.5）三个 Move 里，冷却不可直接对齐。改成 0.15 是一个手感决定，不是转录修正——留给所有者试玩后定。
