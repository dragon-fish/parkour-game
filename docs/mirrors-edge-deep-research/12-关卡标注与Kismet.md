...
- **判定窗口只有跨线那一 tick**，之后不再回头。跳歪了或水平速度太快就没有挽回机会——这
  正是原作的态度：每个动作都在起跳前就该想好，空中操控力只有 0.2，允许扭头是为了看清
  落点、想好下一步，不是为了修正这一步。
- **软垫不交还控制权**，只改变结局。玩家照样失控落到底，照样吃一次 Landing 硬直，区别
  只在不死、不扣血、不播 `LiftAir`。所以 `SoftLandingMove` 是独立状态而不是
  `FallUncontrolled` 的一个开关；两者的钳制几乎一致，可以理解为
  `FallUncontrolled` 在 `SoftLanding` 之上多加了「置死 + 布娃娃 + 濒死后处理」。
