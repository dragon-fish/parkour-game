...
| `LedgeWalk` | `SpeedModifier = 0.1`，兴趣点机制（[12](12-关卡标注与Kismet.md) 里 `TdLedgeWalkVolume` ×2 确证）。✅ owner 实测：**一代永远背对墙**，只有一种朝向；Catalyst 才按入墙角度分成背对/面对两种。所以一代版本只需要**一个**侧向挪步片段——⚠️ 但库里一个都没有，`ClimbLedge` 是翻上去的动作不是侧行，拿前进的走路片段配横向位移会变成螃蟹滑行。📌 owner：几何上玩家本来就走得过去，这个兴趣点**存在的意义就是逼你慢下来**——`SpeedModifier = 0.1` 是全游戏最狠的减速，和 Balance 的 0.34 是同一根节拍棒。所以它是**关卡节奏工具**而不是移动能力，没有节奏需要它的时候，做了也没有用处 |
