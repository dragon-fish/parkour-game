...
| `LedgeWalk` | `SpeedModifier = 0.1`，兴趣点机制（[12](12-关卡标注与Kismet.md) 里 `TdLedgeWalkVolume` ×2 确证）。✅ owner 实测：**一代永远背对墙**，只有一种朝向；Catalyst 才按入墙角度分成背对/面对两种。所以一代版本只需要**一个**侧向挪步片段——⚠️ 但库里一个都没有，`ClimbLedge` 是翻上去的动作不是侧行，拿前进的走路片段配横向位移会变成螃蟹滑行 |
