...
`Walking` `Jump` `Falling` `FallUncontrolled` `SoftLanding` `Landing` `LayOnGround`
`SkillRoll` `Coil` `Slide` `RampSlide` `Crouch` `SpeedVault` `IntoGrab` `Grab`
`WallRun` `WallClimb` `Turn180` `Zipline` `Swing` `Ladder` `Balance` `LedgeWalk`
`SpringBoard` `DodgeJump`

✅ **`LayOnGround` 已实现为独立状态**（`LayOnGroundMove`）：仰面倒地，玩家按跳或前进后
鲤鱼打挺起身。三个入口——

- 📣 向前跳跃时在空中按 Q 转身，落地即仰面倒地。条件只有「起跳时在向前走」，与快慢
  无关：原地起跳转身、向后跳转身都正常站着落地。
- 关卡脚本：`SeqAct_TdFallOnBack`（全游戏只有两个，都在 Subway：坠落电梯的井底触发器、
  列车之旅终点的撞停）。节点本身没有任何属性，扣血由它旁边的 `CauseDamage` 负责。
  本项目对应 `Status.Effect.KNOCKDOWN`。
- 被敌人推倒：本项目没有敌人，未做。

它不是 `Landing`：`Landing` 是站着的、到点自己结束的硬直；这个会顺着惯性滑出去
（CDO `FrictionModifier = 0.15`），并且一直躺到玩家要起来为止。
...
而不是需要修的。⚠️ **这个状态自己不扣任何速度能量**，入场水平速度按
`InertiaConservation = 0.3` 缩放后与冲量相加——逐帧实测与两条被推翻的旧读法见
[04 §4.5](04-墙面动作.md#45-墙面相关的其他跳跃)，触发门槛的读法也在那里。
...
`Barge` / `AirBarge` · `Interact`

📣 `Barge` / `AirBarge` **不是战斗动作**（游戏内实测）：按下攻击键后，射线检测到即将
撞上玻璃时触发，地面和空中各一个，两个都是撞碎玻璃。本项目的玻璃是跑进去就碎
（`BreakableGlass`），暂不实现这两个动作。
