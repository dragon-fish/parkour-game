...
| `DodgeJump` | [04](04-墙面动作.md) + feel-backlog §69。⚠️ 冲量必须在进入那一帧解算成世界向量并锁死。⚠️ 它首先是**战斗动作**（A/D + 空格躲攻击），本项目要它是为了社区那条 side-jump boost —— `JumpAddXY = 600` 配合空中转向可以绕过 7 秒加速曲线 |
| `Balance` | [05 §5.6](05-动作库总览.md#56-balance平衡木)，含 HUD 实测的 8.81 km/h。`InterestLine.Kind` 里已经有 `BALANCE` 枚举 |
| `LedgeWalk` | `SpeedModifier = 0.1`，兴趣点机制（[12](12-关卡标注与Kismet.md) 里 `TdLedgeWalkVolume` ×2 确证）。✅ owner 实测：**一代永远背对墙**，只有一种朝向；Catalyst 才按入墙角度分成背对/面对两种。所以一代版本只需要**一个**侧向挪步片段——✅ 动画有：`Walk_L_Loop` / `Walk_R_Loop`（UAL2 八方向全套，私仓层），就是走路节奏的侧向挪步，正是要的东西——`character_animator.gd` 的 `DIRECTION_SETS` 早就在用这套 `_L`/`_R` 后缀了。📌 owner：几何上玩家本来就走得过去，这个兴趣点**存在的意义就是逼你慢下来**——`SpeedModifier = 0.1` 是全游戏最狠的减速，和 Balance 的 0.34 是同一根节拍棒。所以它是**关卡节奏工具**而不是移动能力，没有节奏需要它的时候，做了也没有用处 |
