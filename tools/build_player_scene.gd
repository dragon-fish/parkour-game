...
# belongs IN THIS FILE, or the next regeneration silently discards it -- with
# ONE deliberate exception: the visible character body and everything that
# depends on it (its AnimationTree, CharacterAnimator). See BodyRoot's own
# comment below for why those specifically must NOT be built here, and
