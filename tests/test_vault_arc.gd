...
func _peak_feet(obstacle_top: float, entry_feet: float, landing_feet: float,
		cfg: SpeedVaultConfig, standing: float) -> float:
	var half: float = standing * 0.5
	var wanted: float = obstacle_top - cfg.vault_over_peak_below_top
...
func test_a_low_box_no_longer_lifts_the_body_a_whole_metre() -> void:
	...
	var cfg := SpeedVaultConfig.new()
	var peak: float = _peak_feet(1.0, 0.0, 0.0, cfg, 1.8)
	assert_lt(peak, 0.3, "the feet still peak %.2f m up over a 1 m box" % peak)
	# And what matters is where the EYE ends up: 1.66 above the feet.
	var eye_above_top: float = peak + 1.66 - 1.0
...
func test_the_eye_clears_by_the_same_amount_at_every_height() -> void:
	# The point of subtracting rather than adding: one number that is right at
	# more than one obstacle. 1.66 - 0.87 = 0.79, whatever the obstacle is --
	# until the floor kicks in below.
	var cfg := SpeedVaultConfig.new()
	for top in [1.0, 1.2, 1.48]:
		var peak: float = _peak_feet(top, 0.0, 0.0, cfg, 1.8)
		var clearance: float = peak + 1.66 - top
		assert_almost_eq(clearance, 0.79, 0.01,
			"a %.2f m obstacle clears by %.2f m" % [top, clearance])
...
func test_an_obstacle_too_low_to_arc_over_gets_no_arc() -> void:
	...
	var cfg := SpeedVaultConfig.new()
	var peak: float = _peak_feet(0.64, 0.0, 0.0, cfg, 1.8)
