...
func test_ground_speed_is_faster_than_walk() -> void:
	...
	assert_gt(c.ground_speed, c.walk_speed, "ground_speed must be faster than the walk modifier")

func test_air_control_is_weaker_than_ground_control() -> void:
	await step(1)
	var c := MovementConfig.new()
	# The whole "commit to your jump" feel depends on this ordering.
	assert_gt(c.ground_accel, c.air_accel, "ground acceleration must exceed air acceleration")

func test_fov_widens_with_speed() -> void:
	await step(1)
	var c := MovementConfig.new()
	assert_gt(c.fov_max, c.fov_base, "max FOV must exceed base FOV")

func test_terminal_velocity_exceeds_jump_velocity() -> void:
	await step(1)
	var c := MovementConfig.new()
	assert_gt(c.terminal_velocity, c.jump_velocity, "terminal velocity must exceed jump velocity")
...
func test_camera_head_follow_strength_defaults_low_and_in_range() -> void:
	await step(1)
	var c := MovementConfig.new()
	assert_true(c.camera_head_follow_strength >= 0.0, "camera_head_follow_strength must not default negative")
	assert_true(c.camera_head_follow_strength <= 1.0, "camera_head_follow_strength must not default above 1.0")
	assert_true(c.camera_head_follow_strength < 0.5, \
