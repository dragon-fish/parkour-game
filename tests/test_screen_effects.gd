...
func test_it_starts_neutral() -> void:
	var fx := _effects()
	await step(1)
	assert_almost_eq(fx.tint_amount, 0.0, 0.0001, "tint is not neutral at rest")
	assert_almost_eq(fx.desaturation, 0.0, 0.0001, "desaturation is not neutral at rest")
	assert_almost_eq(fx.blur, 0.0, 0.0001, "blur is not neutral at rest")
	fx.queue_free()
	await step(1)
...
func test_each_channel_is_independent() -> void:
	...
	assert_almost_eq(fx.tint_amount, 0.0, 0.0001, "setting desaturation moved the tint")
	assert_almost_eq(fx.blur, 0.0, 0.0001, "setting desaturation moved the blur")
	assert_almost_eq(fx.desaturation, 1.0, 0.0001, "desaturation did not take")
	fx.queue_free()
	await step(1)
...
func test_values_are_clamped() -> void:
	...
	assert_almost_eq(fx.desaturation, 1.0, 0.0001, "desaturation exceeded 1")
	fx.set_blur(-0.2)
	assert_almost_eq(fx.blur, 0.0, 0.0001, "blur went below 0")
	fx.queue_free()
	await step(1)
...
func test_tint_set_before_ready_survives_entering_the_tree() -> void:
	...
	assert_almost_eq(c.r, 0.0, 0.0001, "tint red channel did not survive entering the tree")
	assert_almost_eq(c.g, 1.0, 0.0001, "tint green channel did not survive entering the tree")
	assert_almost_eq(c.b, 0.0, 0.0001, "tint blue channel did not survive entering the tree")
	assert_almost_eq(fx.tint_amount, 0.5, 0.0001, "tint amount did not survive entering the tree")
	fx.queue_free()
	await step(1)
