...
func test_1_top_speed_needs_more_than_five_seconds_of_running() -> void:
	...
	assert_true(energy.cap() < pawn.ground_speed - 0.05, \
...
func test_2_air_speed_is_effectively_uncapped_and_air_control_is_tiny() -> void:
	var pawn := PawnConfig.new()
	assert_gt(pawn.air_speed, pawn.ground_speed * 3.0, "air speed is capped near ground speed")
	assert_true(pawn.accel_rate * pawn.air_control < 2.0, "air control is not almost nothing")
...
func test_5_a_key_move_spans_at_least_three_times() -> void:
	...
	assert_gt(best / worst, 3.0, "no key move has a 3x execution gradient")
...
func test_6_landing_is_judged_on_a_resettable_counter() -> void:
	...
	assert_gt(tracker.fall_height, 3.0, "the counter did not measure the drop")
	tracker.reset(6.0)
	assert_almost_eq(tracker.fall_height, 0.0, 0.0001, "the counter is not resettable")

func test_8_moves_declare_their_own_camera_constraints() -> void:
	var config := MovementConfig.new()
	assert_true(config.wall_run.constrain_look, "wall running declares no look constraint")
	assert_true(config.slide.constrain_look, "sliding declares no look constraint")
	assert_true(not config.walking.constrain_look, "walking wrongly constrains the look")
...
func test_10_a_flat_jump_hangs_for_about_four_fifths_of_a_second() -> void:
	...
	assert_almost_eq(hang, 0.7875, 0.02, "flat-jump hang time is not the measured 0.79 s")
	var apex: float = pawn.base_jump_z * pawn.base_jump_z / (2.0 * pawn.gravity)
	assert_true(apex < pawn.skill_roll_landing_height, \
...
func test_8b_an_absolute_yaw_fan_stays_pinned_to_the_facing_the_move_began_with() -> void:
	...
	assert_almost_eq(player.rotation.y, expected, 0.001, \
	...
	for i in 800:
		rig.apply_look(Vector2(100.0, 0.0), player)
	assert_almost_eq(player.rotation.y, reference - deg_to_rad(90.0), 0.001, \
