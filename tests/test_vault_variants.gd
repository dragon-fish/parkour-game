...
func test_the_table_has_all_six_confirmed_variants() -> void:
	...
	for expected in ["auto_step_up_right_leg", "step_up_right_leg_88", "vault_onto", \
		...
		assert_true(names.has(expected), "variant %s missing from the table" % expected)
...
func test_running_fast_at_a_sweet_spot_obstacle_earns_speed() -> void:
	...
	assert_true(variant.has("name"), "no variant matched a fast sweet-spot approach")
	assert_true(variant["name"] == "vault_over", "a fast approach did not pick vault_over")
...
func test_a_high_obstacle_requires_already_being_on_the_way_up() -> void:
	...
	assert_true(cfg.pick_variant(1.7, false, 2.0, 6.0).has("name"), \
...
func test_a_high_obstacle_clamps_speed_down() -> void:
	...
	assert_true(variant["clamp_speed_max"] < 7.0, "the high variant did not clamp speed down")
...
func test_the_lookahead_window_is_time_not_distance() -> void:
	...
	assert_true(cfg.should_commit(2.0, 7.2, variant), "a fast approach did not commit at 2.0 m")
	assert_true(not cfg.should_commit(2.0, 4.0, variant), "a slow approach committed at 2.0 m anyway")

func test_a_stationary_approach_never_commits() -> void:
	# distance / speed must not divide by zero into an infinite lookahead.
	var cfg := _cfg()
	var variant := cfg.pick_variant(1.0, true, 0.0, 6.0)
	assert_true(not cfg.should_commit(1.0, 0.0, variant), "a standstill committed to a vault")
