...
func test_a_fatal_fall_plays_the_death_sequence_before_respawning() -> void:
	...
	for i in 300:
		...
		if arena._death_sequence._playing:
			break
	assert_true(arena._death_sequence._playing, \
		"the death sequence never started after a fatal landing")
	assert_true(not saw_reset, \
	...
	assert_true(not arena._death_sequence._playing, \
	...
	assert_almost_eq(player.global_position.x, arena.spawn_point.global_position.x, 0.5, \
		"the player did not return to the spawn point on the X axis")
	assert_almost_eq(player.global_position.y, arena.spawn_point.global_position.y, 0.5, \
		"the player did not return to the spawn point on the Y axis")
	assert_almost_eq(player.global_position.z, arena.spawn_point.global_position.z, 0.5, \
...
func test_a_manual_reset_mid_cutscene_is_not_undone_when_the_cutscene_would_have_ended() -> void:
	...
	assert_true(not arena._resetting_physics, \
	...
	assert_true(not respawned_again, \
