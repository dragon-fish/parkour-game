...
## The bottom-of-screen subtitle layer -- the game talking to the player.
...
func _build_moves() -> void:
	...
	var table := [
		...
		[Move.TURN_180_IN_AIR, Turn180InAirMove.new(), config.turn_180_in_air],
...
func _drive_body_yaw(delta: float, input: MoveInput) -> void:
	...
	# reason for it to.
	var first_person: bool = camera_rig == null or not camera_rig.in_third_person()
	...
	if not frozen and moving:
		...
		if not frozen and first_person:
...
# --- the step round --------------------------------------------------------------
#
# A first-person body standing still whose view has gone past
# turn_in_place_angle_deg takes ONE step round by that much, toward the view.
...
## How long the heading takes to come round, seconds.
func turn_in_place_window() -> float:
	return config.pawn.turn_in_place_time
