...
# stops reading as head-shaking on a large-headed model (✅ the owner:
# 把她跑步动画的头部左右晃动减弱一半). Runs BEFORE the spring simulators so
# hair reacts to the damped head, and before Player's HeadLook, whose own
# look-at lands on top at full strength.
