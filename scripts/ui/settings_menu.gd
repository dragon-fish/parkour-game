...
const _SWITCH_KEYS := ["small_third_person_body", "occlusion_culling", "physics_props"]
...
const _ROWS := [
	...
	{"key": "physics_props", "label": "物理模拟", "desc": "布料会随风飘动、被人推开，纸箱可以撞开。关闭后布料停在原本的形状，纸箱不再移动，能提高帧数。"},
